...
import { getSingerPicBySingerName } from '@/utils/musicSdk/singerPic'
import {
  clearPlayedList,
  addPlayedList,
  removePlayedList,
...
const handleRestorePlay = async(restorePlayInfo: LX.Player.SavedPlayInfo) => {
  ...
  void getSingerPicBySingerName(musicInfo.singer).then((url) => {
    if (!url || musicInfo.id != playMusicInfo.musicInfo?.id) return
    setSingerPic(url)
  })
...
const debouncePlay = debounceBackgroundTimer((musicInfo: LX.Player.PlayMusic) => {
  ...
  void getSingerPicBySingerName(musicInfo.singer).then((url) => {
    if (!url || musicInfo.id != playerState.playMusicInfo.musicInfo?.id) return
    setSingerPic(url)
  })
