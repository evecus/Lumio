...
const reconcileOrphanGroupSources = async(): Promise<void> => {
  ...
  // 若正在使用的源恰好是孤儿，删完后切换到剩余的第一个可用源
  const wasActiveOrphan = orphanIds.includes(settingState.setting['common.apiSource'])
  await removeUserApi(orphanIds)
  if (wasActiveOrphan) {
    const restApis = await getUserApiList()
    setApiSource(restApis[0]?.id ?? '')
  }
