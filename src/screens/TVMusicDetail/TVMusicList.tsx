 ...
 * TV 分页两列歌曲列表
 *
 * 布局：左列 1-5，右列 6-10，共 10 首/页（每列 5 行）
 *
 * 行内左右键逻辑：
 *   主按钮 → 右键 → 同行 ···
 *   ···   → 左键 → 同行主按钮
...
export interface OnlineMusicListType {
  ...
  /** 菜单键：若当前有行主按钮/三点按钮聚焦，触发该行菜单弹窗，返回 true；否则返回 false */
...
export const OnlineMusicList = forwardRef<OnlineMusicListType, OnlineMusicListProps>((
  ...
  useImperativeHandle(ref, () => ({
    ...
    tryOpenMenuForFocused() {
      for (const row of Object.values(itemRefs.current)) {
        if (row.leftItem?.tryOpenMenuForFocused()) return true
        if (row.rightItem?.tryOpenMenuForFocused()) return true
      }
      return false
    },
  }))
...
const styles = StyleSheet.create({
  ...
  // grid 高度按每行固定高度 x 行数算出固定值，不再用 flex:1 抢占父容器剩余空间，
  // 避免"行固定高度 + 容器弹性伸缩"互相打架导致内容被挤出可视区域或和分页栏重叠
  grid: { height: ITEM_HEIGHT * COL_SIZE, flexDirection: 'row' },
