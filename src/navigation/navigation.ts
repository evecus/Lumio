...
export type TVMusicDetailParams =
  | { type: 'leaderboard'; id: string; name: string; source: LX.OnlineSource }
  | { type: 'songlist'; id: string; name: string; source: LX.OnlineSource }
  | { type: 'mylist'; id: string; name: string }

export function pushTVMusicDetailScreen(componentId: string, params: TVMusicDetailParams) {
  const theme = themeState.theme
  requestAnimationFrame(() => {
    void Navigation.push(componentId, {
      component: {
        name: TV_MUSIC_DETAIL_SCREEN,
        passProps: { params },
        options: {
          ...
          animations: {
            push: {
              content: {
                translationX: { from: windowSizeTools.getSize().width, to: 0, duration: 250 },
              },
            },
            pop: {
              content: {
                translationX: { from: 0, to: windowSizeTools.getSize().width, duration: 250 },
              },
            },
          },
        },
