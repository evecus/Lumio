...
export type RowInfoType = 'full' | 'medium' | 'single' | 'double'

export const getRowInfo = (type: RowInfoType = 'full'): RowInfo => {
  if (type == 'single') return { rowNum: undefined, rowWidth: '100%' }
  // 固定两列，不依赖设备真实宽高比判断（用于画布内固定布局的场景，
  // 比如 TV 搜索页——那里两列与否是页面设计决定的，不应该随设备而变）
  if (type == 'double') return { rowNum: 2, rowWidth: '50%' }
