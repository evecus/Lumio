...
// 歌名归一化：聚合返回的歌名有时会内嵌歌手后缀（如 “xxx - 歌手”），两侧统一去掉末尾段
...
return this.filterData2(datas)
  ...
  createGetListDetail2Task(id, total) {
    ...
    while (total > 0) {
      ...
      total -= limit
      const params = 'need_sort=1&submodule=8&module=CloudMusic&clientver=20809&c_userid=0&pagesize=' + limit + '&need_rd=1&global_collection_id=' + id + '&mode=1&extend_fields=sy_skin,hot_cmt,skin&area_code=1&begin_idx=' + begin_idx + '&appid=1005'
      begin_idx += limit
      tasks.push(this.createHttp(`https://mcloudservice.kugou.com/pubsongsproxy/v2/get_other_list_file_nofilt?${params}&signature=${signatureParams(params, 'android')}`, {
        headers: {
          mid: '207400915044509771309235134867781509615',
          'User-Agent': 'Android16-AndroidPhone-20809-46-0-SearchMainFragment#958959317/661004247/910038176/857461132-playlist-wifi',
          dfid: '-',
  ...
  async getUserListDetail2(global_collection_id) {
    ...
    let list = this.filterDatav9(songInfo)
  ...
  // v9 API（歌单详情走的接口，字段结构扁平，name 为"歌手 - 歌名"格式）
