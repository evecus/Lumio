...
export const useSingerPic = () => {
  const [value, update] = useState(state.singerPic)

  useEffect(() => {
    global.state_event.on('playerSingerPicChanged', update)
    return () => {
      global.state_event.off('playerSingerPicChanged', update)
    }
  }, [])
