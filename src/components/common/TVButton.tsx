...
const useFocusColor = () => {
  ...
  // 提取 rgb 分量，生成半透明背景色，交由原生层与焦点边框共用同一个圆角矩形绘制，
  // 避免 RN style.backgroundColor 不感知原生 borderRadius 而导致背景溢出边框
  const rgba = colorStr.match(/rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/)
  const focusBgColor = rgba
    ? parseColorToInt(`rgba(${rgba[1]}, ${rgba[2]}, ${rgba[3]}, 0.15)`)
    : parseColorToInt('rgba(52, 152, 219, 0)')
  return { focusColor: parseColorToInt(colorStr), focusBgColor }
}
...
export default forwardRef<TVButtonType, Props>((((({
  ...
  const { focusColor, focusBgColor } = useFocusColor()
  ...
  return (
    <AnimatedTvFocusView
      ...
      focusBgColor={isFocused ? focusBgColor : 0x00000000}
      style={[
